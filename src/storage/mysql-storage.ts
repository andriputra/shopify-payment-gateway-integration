...
class MysqlPaymentRedirectRepository implements PaymentRedirectStore {
  ...
  async getBySwipeRequestId(shop: string, swipeRequestId: string): Promise<PaymentRedirectRecord | undefined> {
    ...
    const [rows] = await this.pool.query<RowDataPacket[]>(
      "SELECT * FROM payment_redirects WHERE shop = ? AND swipe_request_id = ? LIMIT 1",
      [shop, want]
    );
    return rows[0] ? this.mapRow(rows[0]) : undefined;
  }
  ...
  private mapRow(row: RowDataPacket): PaymentRedirectRecord {
    return {
      ...
      returnUrlAfterPaid:
        row.return_url_after_paid != null ? String(row.return_url_after_paid) : undefined,
      forwardWebhookUrl:
        row.forward_webhook_url != null ? String(row.forward_webhook_url) : undefined,
      forwardWebhookSecret:
        row.forward_webhook_secret != null ? String(row.forward_webhook_secret) : undefined
    };
  }
}
...
export function createMysqlStorage(): StorageBundle {
  ...
  return {
    initialize: async () => {
      ...
      await ignoreDuplicateColumn(
        pool,
        "ALTER TABLE payment_redirects ADD COLUMN return_url_after_paid TEXT NULL"
      );
      await ignoreDuplicateColumn(
        pool,
        "ALTER TABLE payment_redirects ADD COLUMN forward_webhook_url TEXT NULL"
      );
      await ignoreDuplicateColumn(
        pool,
        "ALTER TABLE payment_redirects ADD COLUMN forward_webhook_secret TEXT NULL"
      );
      await ignoreDuplicateColumn(
        pool,
        "ALTER TABLE payment_redirects ADD COLUMN swipe_request_id VARCHAR(128) NULL"
      );
      await ignoreDuplicateColumn(
        pool,
        "ALTER TABLE payment_redirects ADD COLUMN ws_session_id VARCHAR(128) NULL"
      );

      try {
        await pool.execute(
          "CREATE INDEX idx_payment_redirect_swipe_request_id ON payment_redirects (shop, swipe_request_id)"
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
